...
function initSpinner(t) {
    $(".spinner .add")
        .off()
        .on("click", function () {
            var t = $(this).closest(".spinner").find("input"),
                e = parseInt(t.val());
    ...
    if (
        ($("#shop-page").length &&
            ...
            $(".quickview-trigger").on("click", function () {
                ...
                var t = $(this),
                    e = t.closest(".product-card").attr("data-path"),
                    i = t.closest(".product-card").attr("data-name"),
                    s = parseInt(t.closest(".product-card").attr("data-price")),
                    a = t.closest(".product-card").find("img").attr("src"),
                    o = t.closest(".product-card").attr("data-colors");
                var contact_person = t.closest(".product-card").attr("data-person");
                var contact_number = t.closest(".product-card").attr("data-contact");
                var location = t.closest(".product-card").attr("data-location");
                var description_data = t.closest(".product-card").attr("data-description");
                t.closest(".product-card").attr("data-colors");
                $("#quickview-name").html(i),
                    $(".product-quickview .product-image img").attr("src", a),
                    $("#quickview-price, #quickview-button-price, #price_data").html(s.toFixed(2)),
                    $("#contact_person_view").html(contact_person),
                    $("#contacts").html(contact_number),
                    $("#location_data").html(location),
                    $("#google_map_data").attr("href","https://www.google.com/maps/place/"+location),
                    $("#description_data").html(description_data),
                    setTimeout(function () {
        ...
        $(".products-navigation").length &&
            ($(window).on("scroll", function () {
                $(window).scrollTop() > 65
                    ? $(".products-navigation").addClass("is-active")
                    : ($(".products-navigation").removeClass("is-active"), $(".navigation-panel").fadeOut(), $(".products-navigation .shop-action").removeClass("is-active"));
            }),
            ...
            initComboBox()),
        $("#checkout-button").length &&
            ($("#checkout-button").on("click", function () {
                var t = $(this),
                    e = $("#checkout-step-title"),
                    i = $("#checkout-back"),
                    s = parseInt(t.attr("data-step")),
                    a = parseInt($("#checkout-back").attr("data-step"));
                t.addClass("is-loading"),
                    setTimeout(function () {
                        if ((t.removeClass("is-loading"), $(".checkout-section").removeClass("is-active"), $("#checkout-section-" + s).addClass("is-active"), t.attr("data-step", s + 1), i.attr("data-step", a + 1), 2 === s))
                            e.html("2. Choose a shipping address"), 0 === $(".shipping-address input:checked").length ? t.addClass("is-disabled") : t.removeClass("is-disabled");
                        else if (3 === s) e.html("3. Choose a shipping method"), 0 === $(".shipping-box input:checked").length ? t.addClass("is-disabled") : t.removeClass("is-disabled");
                        else if (4 === s) $(".shipping-logo").addClass("is-active"), e.html("4. Choose a billing address"), 0 === $(".billing-address input:checked").length ? t.addClass("is-disabled") : t.removeClass("is-disabled");
                        else if (5 === s) {
                            ...
                            o.indexOf("navbar-v1")
                                ...
                                : o.indexOf("sidebar-v1") && (window.location.href = "/sidebar-v1-ecommerce-payment.html");
                        }
                    }, 800);
            }),
                ...
                t.addClass("is-loading"),
                    setTimeout(function () {
                        t.removeClass("is-loading"),
                            ...
                            $("#checkout-section-" + i).addClass("is-active"),
                            $("#checkout-button")
                                .removeClass("is-disabled")
                                .attr("data-step", i + 1),
                            t.attr("data-step", i - 1),
                            0 === i
                                ? (window.location.href = "/ecommerce-cart.html")
                                : 1 === i
                                ? e.html("1. Confirm your order")
                                : 2 === i
                                ? (e.html("2. Choose a shipping address"), $(".shipping-logo").removeClass("is-active"))
                                : 3 === i && (e.html("3. Choose a shipping method"), $(".shipping-logo").addClass("is-active"));
                    }, 800);
            }),
            ...
            $(".shipping-box input").on("change", function () {
                ...
                $(".shipping-logo").attr("src", t).addClass("is-active"),
        ...
        var t,
            e = Stripe("pk_test_6pRNASCoBOKtIshFeQd4XMUh"),
            i = e.elements(),
            s = window.localStorage.getItem("theme");
        t = null != s && null != s && "dark" != s ? { base: { fontSize: "14px", color: "#595d6e" } } : { base: { iconColor: "#fff", fontSize: "14px", color: "#fff", "::placeholder": { color: "#6f809e" } } };
